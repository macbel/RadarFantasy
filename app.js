...
const DEFAULT_LEAGUE_PREFERENCES = {
  strictBudget: true,
  riskAverse: false,
  investmentMode: false,
  showImageUpload: false,
...
const readTeamTrackingStorage = () => readJsonStorage(TEAM_TRACKING_KEY, { teams: [], articles: [], updatedAt: null, filter: [] });
...
const writeLeagueFavoritesCacheEntry = (leagueId, favorites, updatedAt = new Date().toISOString()) => {
  ...
  const cache = readLeagueFavoritesCache();
  cache[String(leagueId)] = {
    updatedAt,
    favorites: Array.isArray(favorites) ? favorites.map(cleanWorldcupPlayerIdentity) : []
  };
  writeLeagueFavoritesCache(cache);
  return updatedAt;
};

const resolveLeagueFavorites = (league) => {
  const leagueId = String(league?.id || "");
  const persisted = leagueId ? readLeagueFavoritesCache()[leagueId] : null;
  const remoteUpdatedAt = Date.parse(league?.favoritesUpdatedAt || league?.updatedAt || 0) || 0;
  const localUpdatedAt = Date.parse(persisted?.updatedAt || 0) || 0;
  if (persisted && localUpdatedAt >= remoteUpdatedAt) {
    return {
      favorites: Array.isArray(persisted.favorites) ? persisted.favorites.map(cleanWorldcupPlayerIdentity) : [],
      updatedAt: persisted.updatedAt || league?.favoritesUpdatedAt || null
    };
  }
  return {
    favorites: Array.isArray(league?.favorites) ? league.favorites.map(cleanWorldcupPlayerIdentity) : [],
    updatedAt: league?.favoritesUpdatedAt || null
  };
};

const mergeLeaguePreferences = (basePreferences = {}, leaguePreferences = {}) => ({
  ...DEFAULT_LEAGUE_PREFERENCES,
  ...(basePreferences || {}),
  ...(leaguePreferences || {}),
  rewards: {
    ...(DEFAULT_LEAGUE_PREFERENCES.rewards || {}),
    ...(basePreferences?.rewards || {}),
    ...(leaguePreferences?.rewards || {})
  }
});
...
const fixtureCandidatesForPlayer = (player, fixtures = state.leagueFixtures) => {
  const events = fixtures?.events || [];
  const nowSeconds = Date.now() / 1000;
...
const fixturePlayerCoverage = (fixtures, players = [...state.players, ...state.teamPlayers]) => {
  const unique = new Map();
  players.forEach((player) => {
    const key = String(player?.biwengerPlayerId || player?.id || `${player?.name || ""}-${player?.team || ""}`);
...
const nextMatchForPlayer = (player) => {
  const candidates = fixtureCandidatesForPlayer(player);
...
const incomingOfferPlayerKey = (offer = {}) => {
  const playerId = Number(offer.playerId || 0);
  if (playerId > 0) return `player:${playerId}`;
  const playerName = normalize(offer.playerName || operationPlayerData(offer).name || "");
  return playerName && playerName !== "jugador" ? `name:${playerName}` : `offer:${offerIdKey(offer)}`;
};

const bestIncomingOffersByPlayer = (incoming = []) => {
  const bestByPlayer = new Map();
  incoming.forEach((offer) => {
    const key = incomingOfferPlayerKey(offer);
    const previous = bestByPlayer.get(key);
    const amount = moneyAmount(offer.amount);
    const previousAmount = previous ? moneyAmount(previous.amount) : -1;
    if (!previous || amount > previousAmount || (amount === previousAmount && incomingOfferRank(offer) > incomingOfferRank(previous))) {
      bestByPlayer.set(key, offer);
    }
  });
...
const incomingOfferSummary = (incoming = activeIncomingOffers()) => {
  ...
  const applicable = bestIncomingOffersByPlayer(incoming);
  const total = applicable.reduce((sum, offer) => sum + moneyAmount(offer.amount), 0);
  return {
...
const salePlanKeepsValidLineup = (players, selectedPlayerKeys, nextPlayer) => {
  const nextKey = String(nextPlayer?.biwengerPlayerId || nextPlayer?.id || "");
  const removed = new Set([...selectedPlayerKeys, nextKey]);
  const remaining = players
    .filter((player) => player.position !== "ENT")
    .filter((player) => !removed.has(String(player.biwengerPlayerId || player.id || "")));
  if (remaining.length < 11) return false;
  const byPosition = remaining.reduce((counts, player) => {
...
const assistantSaleRows = (players = assistantTeamPlayers(), context = {}) => {
  const evaluated = players
    ...
    .filter((row) => Number(row.sale.value || 0) > 0);
  const ordinary = evaluated
    .filter((row) => row.sale.action !== "Mantener" && row.sale.score >= 54)
    .sort((a, b) => b.sale.score - a.sale.score || b.sale.value - a.sale.value);
  const balance = Number.isFinite(Number(context.balanceAfterRoundAndOffers))
    ? Number(context.balanceAfterRoundAndOffers)
    : Number(state.biwengerOperations?.finance?.balance ?? state.finance.balance);
  const debt = Number.isFinite(balance) ? Math.max(0, -balance) : 0;
  if (debt <= 0) return ordinary.slice(0, 6);

  const selected = [];
  const selectedPlayerKeys = new Set();
  let potential = 0;
  const addIfSafe = (row, forced = false) => {
    const playerKey = String(row.player?.biwengerPlayerId || row.player?.id || "");
    if (!playerKey || selectedPlayerKeys.has(playerKey) || row.existingSale) return;
    if (row.sale.reason.includes("jornada en curso") || !salePlanKeepsValidLineup(players, selectedPlayerKeys, row.player)) return;
    const sale = forced ? {
      ...row.sale,
      score: Math.max(76, row.sale.score),
      action: "Vender hoy",
      reason: `saldo negativo: necesitas cubrir ${formatFinanceMoney(Math.max(0, debt - potential))} · ${row.sale.reason}`
    } : row.sale;
    selected.push({ ...row, sale });
    selectedPlayerKeys.add(playerKey);
    potential += Number(sale.suggestedPrice || 0);
  };

  ordinary.forEach((row) => {
    if (selected.length < 6 && potential < debt) addIfSafe(row);
  });
  evaluated
    .filter((row) => !row.existingSale && !selectedPlayerKeys.has(String(row.player?.biwengerPlayerId || row.player?.id || "")))
    .sort((a, b) => Number(Boolean(a.sale.recent?.hot)) - Number(Boolean(b.sale.recent?.hot))
      || a.sale.quality - b.sale.quality
      || b.sale.value - a.sale.value)
    .forEach((row) => {
      if (selected.length < 6 && potential < debt) addIfSafe(row, true);
    });
  return selected;
};
...
const assistantOfferRows = (incoming, myOffers, futureIncome = 0) => {
  ...
  const applicableIncoming = bestIncomingOffersByPlayer(incoming);
  const recommended = chooseRecommendedOfferSet(applicableIncoming, targetAmount);
  const recommendedIds = new Set(recommended.map(offerIdKey));
  return applicableIncoming.map((offer) => {
...
const assistantPlanSnapshot = () => {
  ...
  const allOfferAmount = bestIncomingOffersByPlayer(incoming).reduce((sum, offer) => sum + moneyAmount(offer.amount), 0);
  const saleContext = {
    baseBalance: balance,
  ...
  const salePotential = sales
    .filter((row) => row.sale.action !== "Mantener" && !row.existingSale)
  ...
  target.innerHTML = `
    <div class="assistant-hero">
      ...
      <div class="assistant-metrics">
        ...
        <div><span>Ofertas recibidas</span><strong>${incoming.length} · máximo aplicable ${formatFinanceMoney(allOfferAmount)}</strong></div>
        <div><span>Aceptando las mejores</span><strong class="${projectedAllOffers < 0 ? "negative" : "positive"}">${formatFinanceMoney(projectedAllOffers)}</strong></div>
    ...
    <div class="assistant-grid">
      <section class="assistant-card">
        ...
        `).join("") : `<p class="muted-empty compact">${Number.isFinite(balance) && balance < 0 ? "Las nuevas pujas se activarán cuando las ventas u ofertas aceptadas dejen saldo disponible." : "El asistente no ve una puja que compense riesgo, precio y encaje."}</p>`}
      </section>
      <section class="assistant-card">
...
const toggleFavoritePlayer = async (player) => {
  ...
  state.favoriteStateUpdatedAt = writeLeagueFavoritesCacheEntry(state.activeLeagueId, state.favorites);
...
const applyTeamTrackingPayload = (payload = {}) => {
  ...
  const allowedKeys = new Set(state.trackedTeams.map((team) => trackedTeamKey(team)));
  state.trackedTeamFilter = Array.isArray(payload.filter)
    ? payload.filter
...
const renderTrackedTeamFilter = () => {
  const target = qs("#team-tracking-filter");
  if (!target) return;
  if (state.trackedTeams.length <= 1) {
    target.hidden = true;
    target.innerHTML = "";
    return;
  }
  target.hidden = false;
  target.innerHTML = state.trackedTeams.map((team) => {
    const active = state.trackedTeamFilter.length ? state.trackedTeamFilter.some((item) => trackedTeamKey(item) === trackedTeamKey(team)) : false;
    return `<button class="ghost-button team-tracking-filter-chip ${active ? "active" : ""}" type="button" data-team-tracking-filter="${escapeHtml(team)}" aria-pressed="${active ? "true" : "false"}">${escapeHtml(team)}</button>`;
  }).join("");
};
...
const renderTrackedTeamFeed = () => {
  ...
  if (!target) return;
  const visibleArticles = visibleTrackedTeamArticles();
  if (total) total.textContent = `${visibleArticles.length} noticia${visibleArticles.length === 1 ? "" : "s"}`;
  target.innerHTML = visibleArticles.length
    ...
    `).join("")
    : `<p class="muted-empty">${state.trackedTeams.length ? (state.trackedTeamFilter.length ? "No hay titulares visibles con el filtro actual. Marca otro equipo o desactiva el filtro." : "Todavía no han llegado titulares para los equipos seguidos. Pulsa Actualizar noticias.") : "Añade equipos y aparecerán aquí las últimas noticias filtradas."}</p>`;
};
...
const removeTrackedTeam = async (teamName) => {
  ...
  state.trackedTeamFilter = state.trackedTeamFilter.filter((team) => trackedTeamKey(team) !== key);
  state.trackedTeamsUpdatedAt = new Date().toISOString();
  renderTeamTracking();
...
const toggleTrackedTeamFilter = (teamName) => {
  const normalized = sanitizeTrackedTeamName(teamName);
  if (!normalized) return;
  const key = trackedTeamKey(normalized);
  const exists = state.trackedTeamFilter.some((team) => trackedTeamKey(team) === key);
  state.trackedTeamFilter = exists
    ? state.trackedTeamFilter.filter((team) => trackedTeamKey(team) !== key)
    : [...state.trackedTeamFilter, normalized];
  writeTeamTrackingStorage({
    teams: state.trackedTeams,
    articles: state.trackedTeamArticles,
    updatedAt: state.trackedTeamsUpdatedAt,
  ...
  setTeamTrackingStatus(
    state.trackedTeamFilter.length
      ? `Filtro activo: ${state.trackedTeamFilter.join(", ")}.`
...
const applyLeague = (league) => {
  ...
  state.preferences = mergeLeaguePreferences({}, league.preferences || {});
  ...
  if (state.favoriteStateUpdatedAt) {
    writeLeagueFavoritesCacheEntry(league.id, state.favorites, state.favoriteStateUpdatedAt);
  }
...
const chooseRecommendedOfferSet = (incoming, targetAmount) => {
  ...
  const decorated = bestIncomingOffersByPlayer(incoming)
    .map((offer) => ({ offer, metrics: offerSportCost(offer), amount: moneyAmount(offer.amount) }));
...
const renderOfferSimulation = (incoming, myOffers) => {
  ...
  const selectedOffers = bestIncomingOffersByPlayer(incoming.filter((offer) => state.offerSimulation.selectedOfferIds.includes(offerIdKey(offer))));
  state.offerSimulation.selectedOfferIds = selectedOffers.map(offerIdKey);
  ...
  return `
    <section class="offer-simulator">
      ...
      <div class="operation-metrics offer-simulation-metrics">
        <div><span>Total ofertas</span><strong>${incoming.length} · máximo aplicable ${formatFinanceMoney(incomingSummary.total)}</strong></div>
        <div class="${incomingSummary.balanceAfterAll < 0 ? "danger" : ""}"><span>Mejor por jugador</span><strong>${formatFinanceMoney(incomingSummary.balanceAfterAll)}</strong></div>
...
const renderBiwengerOperations = () => {
  ...
  bidsTarget.innerHTML = `
    <div class="operation-metrics">
      ...
      <div><span>Ofertas recibidas</span><strong>${incoming.length} · máximo aplicable ${formatFinanceMoney(incomingSummary.total)}</strong></div>
      <div class="${incomingSummary.balanceAfterAll < 0 ? "danger" : ""}"><span>Mejor por jugador</span><strong>${formatFinanceMoney(incomingSummary.balanceAfterAll)}</strong></div>
      ...
      <div class="${futureWithAllOffersAndRound < 0 ? "danger" : ""}"><span>Ofertas + jornada</span><strong>${formatFinanceMoney(futureWithAllOffersAndRound)}</strong><small>Mejor oferta por jugador y ganando pujas</small></div>
    </div>
  ...
  bidsTarget.querySelectorAll(".offer-sim-checkbox").forEach((checkbox) => checkbox.addEventListener("change", () => {
    ...
    if (checkbox.checked) {
      const selectedOffer = incoming.find((offer) => offerIdKey(offer) === checkbox.value);
      const playerKey = selectedOffer ? incomingOfferPlayerKey(selectedOffer) : "";
      if (playerKey) incoming.forEach((offer) => {
        if (incomingOfferPlayerKey(offer) === playerKey) selected.delete(offerIdKey(offer));
      });
...
const loadLiveRound = async (showFeedback = true) => {
  const target = qs("#live-round-ranking");
  if (!shouldShowExperimentalLiveRound()) {
    if (target) target.innerHTML = `<p class="muted-empty">La jornada fantasy experimental está oculta porque Biwenger no ofrece datos suficientemente fiables en todas las ligas.</p>`;
    return null;
  }
  if (!state.biwenger.connected) {
...
const ensureLiveRoundForFinance = async (showFeedback = false) => {
  if (!shouldShowExperimentalLiveRound()) return null;
...
const applyVisibilityPreferences = () => {
  ...
  if (liveRoundPanel) liveRoundPanel.hidden = !shouldShowExperimentalLiveRound();
  if (!shouldShowExperimentalLiveRound() && qs('.league-tab.active')?.dataset.leagueTab === "live-round") {
    openLeaguePanel("fixtures");
  }
};
...
const renderSummary = (players, { includeStrategic = true } = {}) => {
  ...
  if (includeStrategic && shouldShowMarketAnalysis()) renderStrategicMarket(players);
  renderFinance();
};
...
const openLeaguePanel = (panelName) => {
  if (panelName === "live-round" && !shouldShowExperimentalLiveRound()) {
    panelName = "fixtures";
  }
...
const updateTopbarForView = (viewName = activeViewName()) => {
  ...
  if (subtitle) subtitle.textContent = `${activeLeagueName() || "Mi liga"} · ${FANTASY_PROVIDER_META[leagueFantasyProvider(activeLeague()) || "local"]?.label || "Local / manual"}`;
  qs("#refresh-sources")?.toggleAttribute("hidden", viewName !== "market");
  qs("#analyze-market")?.toggleAttribute("hidden", viewName !== "market");
...
const refreshLeagueCenterSettingsManually = async ({ forceFixtures = true } = {}) => {
  if (!state.biwenger.connected) {
    setLeagueOperationStatus("Conecta Biwenger para actualizar el centro de liga.", "error");
    return false;
  }
  const fixturesLoaded = await loadLeagueFixtures(false, { forceRefresh: forceFixtures });
...
const refreshAllSettingsManually = async ({ reason = "manual" } = {}) => {
  ...
  const failures = [];
  const attemptRefreshStep = async (statusText, failureLabel, action) => {
    throwIfDataSyncCancelled();
    updateDataSync(statusText);
    try {
      const result = await action();
      if (result === false) throw new Error(failureLabel);
      throwIfDataSyncCancelled();
      return true;
    } catch (error) {
      if (isDataSyncCancellation(error)) throw error;
      failures.push(`${failureLabel}: ${error?.message || "sin respuesta"}`);
      return false;
    }
  };
  ...
  try {
    await attemptRefreshStep("Sincronizando ligas y sesión de Biwenger...", "Ligas y sesión", refreshLeagueSettingsManually);
    ...
    await attemptRefreshStep("Actualizando plantilla de la liga seleccionada...", "Plantilla", refreshTeamSettingsManually);
    await attemptRefreshStep("Actualizando mercado y pujas de la liga seleccionada...", "Mercado", async () => {
      if (!await refreshMarketSettingsManually()) return false;
      await loadBiwengerOperations(false);
      return true;
    });
    // El calendario siempre se intenta, aunque plantilla o mercado fallen, y
    // fuerza red después de disponer de los equipos recién sincronizados.
    await attemptRefreshStep("Actualizando próximos partidos...", "Calendario", () => loadLeagueFixtures(false, { forceRefresh: true }));
    await attemptRefreshStep("Actualizando clasificación y centro de liga...", "Centro de liga", async () => {
      const overviewLoaded = await loadLeagueOverview();
      if (shouldShowExperimentalLiveRound()) await ensureLiveRoundForFinance(false);
      return overviewLoaded;
    });
    await attemptRefreshStep("Actualizando fuentes deportivas...", "Fuentes deportivas", refreshSourcesSettingsManually);
    if (platformUserCanAccess("team") && state.teamPlayers.length) {
      await attemptRefreshStep("Actualizando noticias de los jugadores del equipo...", "Noticias del equipo", () => refreshTeamNews({ force: true }));
    }
    await attemptRefreshStep("Actualizando director deportivo...", "Director deportivo", refreshDailyPlanSettingsManually);
    if (platformUserCanAccess("favorites")) {
      await attemptRefreshStep("Actualizando favoritos y sus noticias...", "Favoritos", () => refreshFavoritesAll({ force: true }));
    }
    if (platformUserCanAccess("team-tracking")) {
      await attemptRefreshStep("Actualizando seguimiento y noticias de equipos...", "Seguimiento de equipos", async () => {
    ...
    renderDailyPlanIfVisible();
    await attemptRefreshStep("Guardando los datos actualizados...", "Guardado final", async () => {
      await notifyDailyPlanIfNeeded();
      await flushActiveLeagueSave();
      return true;
    });
    if (failures.length) {
      const message = `Actualización completada con incidencias en ${failures.map((item) => item.split(":")[0]).join(", ")}. El resto de secciones sí se ha actualizado.`;
      setLeagueOperationStatus(message, "error");
      updateDataSync(message, "error");
      return false;
    }
    updateDataSync("Todas las secciones están actualizadas, incluidos los próximos partidos.", "ready");
  } catch (error) {
...
const initEvents = () => {
  ...
  qs("#show-live-round")?.addEventListener("change", (event) => {
    state.preferences.showExperimentalLiveRound = event.target.checked;
    syncSettingsControls();
  ...
  document.addEventListener("click", (event) => {
    const button = event.target.closest?.("[data-team-tracking-filter]");
    if (!button) return;
    event.preventDefault();
    event.stopPropagation();
